...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, UserAuthenticatorInterface $userAuthenticator, UserAuthenticator $authenticator, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // envoyer un mail en utilisant Messenger. on doit avoir injecté le MessageBus dans ce controller
            $this->messageBus->dispatch (new EmailInscription($user->getId())); // on envoie l'id, pas toute l'entité

            // generate a signed url and email it to the user
            $this->emailVerifier->sendEmailConfirmation('app_verify_email', $user,
                (new TemplatedEmail())
                    ->from(new Address('zyriab@gmail.com', 'Essai'))
                    ->to($user->getEmail())
